/**
 * Darqie Music Player — persistent OBR popover
 *
 * This page is opened by music-bg.js (background_url) as a persistent OBR popover.
 * OBR marks popover iframes with allow="autoplay", so audio and YouTube iframes
 * inside here can play without a user gesture.
 *
 * Handles ALL music playback: Dropbox/mp3 (<audio>), YouTube (iframe), Spotify (iframe).
 * The GM panel writes OBR metadata; this page reads and plays it for everyone.
 */
...
function applyMusic(metadata) {
  ...
  if (!track || !track.url || !state.isPlaying) { stopAll(); return; }
  ...
  // runtimeKey changes only when GM intentionally changes/restarts the track
    ...
    if (currentRuntimeKey !== runtimeKey) {
      ...
      bgAudio.play()
        .then(() => {
          ...
          if (drift > 2) { try { bgAudio.currentTime = positionSec; } catch (_) {} }
        })
        .catch(() => {});
    } else {
      bgAudio.volume = effectiveVolume();
  ...
  // Stop audio when switching to non-audio
  if (bgAudio.src) {
    bgAudio.pause();
    bgAudio.removeAttribute('src');
    bgAudio.load();
...
// ── Volume sync (localStorage storage event) ──────────────────────────────────
...
waitForOBR((OBR) => {
  OBR.onReady(async () => {
    ...
    // Sync volume key with main popup (same origin, same localStorage)
    ...
    // Prefer Supabase snapshot if newer
    if (roomId) {
    ...
    // Safety re-sync every 10 s (covers edge cases like Supabase-only updates)
    setInterval(async () => {
