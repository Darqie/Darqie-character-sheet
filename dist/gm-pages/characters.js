...
export function initPage({ root }) {
  ...
  async function loadRows() {
    ...
    try {
      ...
      rows = dedupeRowsByCharacterName(Array.isArray(data) ? data : []);
    } catch (_) {
      // Не показуємо legacy-дані при недоступній БД, щоб уникнути "воскресіння" видалених персонажів.
      rows = [];
    }
  }
  ...
  async function upsertObrRegistryEntry(characterName, playerName) {
    if (!OBR || !characterName) return;
    ...
    try {
      ...
      if (!found) {
        nextRegistry.push({ characterName, playerName: playerName || '' });
      }
  ...
  async function removeObrRegistryEntry(characterName) {
    if (!OBR || !characterName) return;

    try {
      const metadata = await OBR.room.getMetadata();
      const registry = Array.isArray(metadata?.[DARQIE_REGISTRY_KEY]) ? metadata[DARQIE_REGISTRY_KEY] : [];
      const nextRegistry = registry.filter((entry) => (entry?.characterName || '') !== characterName);
  ...
  async function syncTokenCharacterName(oldName, nextName) {
    if (!OBR || !oldName || !nextName || oldName === nextName) return;
    ...
    const tokenIds = allItems
      .filter((item) =>
        ...
        item.metadata?.characterSheet?.characterName === oldName
      )
      .map((item) => item.id);
    ...
    if (tokenIds.length > 0) {
      await OBR.scene.items.updateItems(tokenIds, (items) => {
        items.forEach((item) => {
          item.name = nextName;
          if (item.text) item.text.plainText = nextName;
          if (item.metadata?.characterSheet) {
            item.metadata.characterSheet.characterName = nextName;
          }
        });
      });
    }
    ...
    const badgeIds = allItems
      .filter((item) =>
        ...
        (item.metadata?.characterName === oldName)
      )
      .map((item) => item.id);
    ...
    if (badgeIds.length > 0) {
      await OBR.scene.items.updateItems(badgeIds, (items) => {
        items.forEach((item) => {
          if (item.metadata) {
            item.metadata.characterName = nextName;
          }
        });
      });
    }
  ...
  async function createCharacter(type, overrides = {}) {
    ...
    if (Array.isArray(inserted) && inserted[0]) {
      ...
      renderTable();
      await upsertObrRegistryEntry(createdRow.character_name, createdRow.player_name || '');
      return createdRow;
    }
    ...
    try {
      if (target.matches('[data-field="character_name"]')) {
        ...
        if (updated) {
          ...
          rowsSignature = buildRowsSignature(rows);
          tr.setAttribute('data-character-name', updated.character_name);
          await syncTokenCharacterName(oldName, updated.character_name);
          await removeObrRegistryEntry(oldName);
          await upsertObrRegistryEntry(updated.character_name, updated.player_name || '');
        }
      }
  ...
  tbody.addEventListener('click', async (event) => {
    ...
    try {
      ...
      if (action === 'delete') {
        ...
        rowsSignature = buildRowsSignature(rows);
        renderTable();
        await removeObrRegistryEntry(row.character_name);
        return;
      }
