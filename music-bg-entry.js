 ...
 * No popover is opened — audio plays directly here.
 * OBR popover was removed because it placed a click-blocking overlay on the main panel.
 *
...
async function resolveYouTubeAudio(ytUrl) {
  ...
  if (cached && Date.now() - cached.ts < YT_CACHE_TTL) {
    console.log('[MusicBG] YT cache hit');
  ...
  try {
    ...
    if (res.ok) {
      ...
      if ((d?.status === 'tunnel' || d?.status === 'redirect') && d.url) {
        ...
        console.log('[MusicBG] Cobalt v10 resolved ✓');
        return d.url;
      }
      console.warn('[MusicBG] Cobalt v10 status:', d?.status, d?.error?.code);
    } else {
      console.warn('[MusicBG] Cobalt v10 HTTP', res.status);
    }
  } catch (e) { console.warn('[MusicBG] Cobalt v10 error:', e.message); }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if ((d?.status === 'stream' || d?.status === 'redirect' || d?.status === 'tunnel') && d.url) {
        ytAudioCache.set(ytUrl, { audioUrl: d.url, ts: Date.now() });
        console.log('[MusicBG] Cobalt v1 resolved ✓');
        return d.url;
      }
      console.warn('[MusicBG] Cobalt v1 status:', d?.status);
    } else {
      console.warn('[MusicBG] Cobalt v1 HTTP', res.status);
    }
  } catch (e) { console.warn('[MusicBG] Cobalt v1 error:', e.message); }
    ...
    for (const base of pipedInstances) {
      try {
        ...
        if (stream?.url) {
          ...
          console.log('[MusicBG] Piped resolved via', base, '✓');
          return stream.url;
        }
      } catch (e) { console.warn('[MusicBG] Piped error:', base, e.message); }
    }
  }

  console.error('[MusicBG] All YouTube resolvers failed:', ytUrl.slice(-40));
  return null;
}
...
let pendingPlay = null; // { src, posSec, loop } — queued while autoplay locked
...
// ── Autoplay unlock ───────────────────────────────────────────────────────────
// background_url may not have allow="autoplay". We attempt play() and if blocked,
// we retry once when the user interacts with any OBR frame via storage event.
...
function attemptPlay(src, posSec, loop) {
  ...
  bgAudio.play().then(() => {
    ...
    console.warn('[MusicBG] play() blocked — will retry on user interaction:', e?.message || e);
    pendingPlay = { src, posSec, loop };
  });
}

// Listen for user-interaction signals written to localStorage by gm-panel/index pages
window.addEventListener('storage', (e) => {
  if (e.key === 'darqie.userInteracted' && pendingPlay) {
    console.log('[MusicBG] User interaction signal received — retrying play');
...
function stopAll()     { console.log('[MusicBG] stopAll'); runtimeKey = ''; pendingPlay = null; stopAudio(); stopSpotify(); }
...
function applyMusic(metadata) {
  ...
  if (type === 'youtube') {
    ...
    if (runtimeKey !== rk) {
      runtimeKey = rk;
      stopAudio();
      console.log('[MusicBG] Resolving YouTube:', track.url.slice(-40));
      resolveYouTubeAudio(track.url).then((audioUrl) => {
        if (!audioUrl || runtimeKey !== rk) return;
        attemptPlay(audioUrl, posSec, repeat);
      }).catch(() => {});
    } else {
      bgAudio.volume = effectiveVol();
    }
  ...
  if (type === 'audio' || type === 'dropbox') {
    ...
    if (runtimeKey !== rk) {
      runtimeKey = rk;
      console.log('[MusicBG] Loading audio:', url.slice(-60));
...
OBR.onReady(async () => {
  ...
  console.log('[MusicBG] roomId:', roomId, '| player:', playerName);
